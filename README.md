...
命令完成后会输出会话链接，默认监听所有 IPv4 网卡（`0.0.0.0`），默认本机入口为：
...
默认监听 `0.0.0.0`，允许不配置证书的 HTTP。默认 Origin 仍为 `http://127.0.0.1:4310`；远程访问需要用 `--origin` 指定实际浏览器地址，Host/Origin 精确校验仍然生效。例如：
 
```powershell
ompw stop
ompw --host 0.0.0.0 --origin "http://192.168.1.10:4310"
```

HTTP 不加密密码、验证码、Cookie 和终端内容，非可信网络建议使用 HTTPS：
...
- 服务配置保存在私有目录的 `service.json`，下次启动继续使用。已有配置中的回环监听不会自动覆盖，需显式传入 `--host 0.0.0.0`。后台运行时不能通过另一条注册命令修改监听配置；先 `ompw stop`。
